...
${noteData.text_content}`;
      ...
      new import_obsidian.Notice(`Created/Updated: ${filePath}`);
    } catch (err) {
  ...
  async sendVaultData(frontmatterData) {
    const submitUrl = this.settings.serverUrl.replace(
      "/joinVault",
      "/submitText"
    );
    try {
      const response = await axios_default.post(submitUrl, frontmatterData, {
        headers: {
          Authorization: `Bearer ${this.settings.joinCode}`,
          "Content-Type": "application/text"
        }
      ...
      new import_obsidian.Notice("Successfully sent vault data to server");
      return response.data;
    } catch (error) {
  ...
  async performFullSync() {
    ...
    try {
      new import_obsidian.Notice("Fetching updates from server...");
      ...
      new import_obsidian.Notice("Preparing local changes...");
      const frontmatterData = await this.getAllFrontmatter();
      new import_obsidian.Notice("Sending local changes to server...");
      await this.sendVaultData(frontmatterData);
      this.settings.lastSyncTimestamp = Date.now();
      await this.saveSettings();
      new import_obsidian.Notice("Full sync completed successfully!");
    } catch (error) {
  ...
  async joinVault(joinCode) {
    try {
      ...
      await this.saveSettings();
      new import_obsidian.Notice("Joining vault and performing initial sync...");
      await this.performFullSync();
      new import_obsidian.Notice("Successfully joined vault and completed initial sync!");
    } catch (error) {
      this.settings.isAuthenticated = false;
      this.settings.joinCode = "";
      await this.saveSettings();
      new import_obsidian.Notice(`Failed to join vault: ${error.message}`);
      throw error;
    }
  }
  async loadSettings() {
